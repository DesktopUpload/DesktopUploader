      ...
      if Assigned(LItem.FieldObject) then begin
        if (LItem.FieldObject is TStream) then begin
          ...
          if (LItem.FieldObject is TStrings) then begin
            AppendString(FInternalBuffer, TStrings(LItem.FieldObject).Text, -1, TIdTextEncoding.UTF8);
            Inc(FCurrentItem);
          end;
        end;
      end else begin
...
begin
  Result := TIdTextEncoding.UTF8.GetByteCount(FormatField);
  if Assigned(FFieldObject) then begin
    if FieldObject is TStrings then begin
      //Result := Result + Length(TStrings(FieldObject).Text) + 2;
      Result := Result + TIdTextEncoding.UTF8.GetByteCount(TStrings(FieldObject).Text) + 2;
    end else begin
